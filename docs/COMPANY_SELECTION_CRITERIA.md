...
This document defines the selection criteria for companies included in the `Toronto AI startup map`.
...
This product is a curated map designed to make the Toronto AI scene easy to understand.
...
2. Their presence in `Toronto` can be verified through a public source
...
`Listed on YC`, `seen on X`, or `Toronto` written in a job post is not enough on its own.
...
3. Its Toronto presence can be verified through a public source
...
1. Companies whose Toronto presence can only be inferred
2. Companies that only say `Toronto, ON` without a strong address or location source
...
3. Companies that seem strongly connected to Toronto, but do not yet have enough public evidence
...
3. Find a public source for the Toronto location
